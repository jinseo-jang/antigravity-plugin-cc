...
# ponytail: allowlist hardcoded (four models). Upgrade path: read from config
...
def check_model(model: str | None) -> str | None:
    ...
    if effective not in _SUPPORTED:
        return (
            f"Unsupported model '{effective}'. agy supports gemini-3.6-flash (default),"
            " gemini-3.5-flash, gemini-3.5-flash-lite, or gemini-3.1-pro-preview."
            " Options: re-run with --model gemini-3.6-flash, or set a default via /agy:setup."
        )
    return None
