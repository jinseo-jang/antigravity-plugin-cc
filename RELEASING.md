...
CLAUDE_CONFIG_DIR="$tmp" claude plugin install agy@agy-plugin-cc
...
| User | What happens |
|---|---|
| **New** | `/plugin marketplace add` + `/plugin install agy@agy-plugin-cc` → first session installs the `v<version>` backend. |
